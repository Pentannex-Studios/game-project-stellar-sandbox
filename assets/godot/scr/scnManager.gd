...
func _loadGameArcRes(_object: Resource) -> void:
	if _object != null:
		# IGNORE: Debug
		print("Adding object ", _object, " as a the game arc ", self)
		
		_gameProper.call_deferred("loadGameArc", _object)
		
		# TODO: Add the arc cutscene loader here.
...
# Resource loader.
func loadResource(_scnPath: String) -> Node:
	var _node: Node = null
	# Progress bar in array to track loading progress.
	var _scnProgress: Array[float] = []
	...
	_scnOutput =  ResourceLoader.load_threaded_get(_scnPath)
	if _scnOutput != null:
		_node = _scnOutput.instantiate()
		
		# IGNORE: Debug
		print("Adding object ", _scnOutput, " is successful.")
		print("#------------------------------------------------------------------------------#")
